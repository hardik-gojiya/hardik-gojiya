...
import { faExternalLinkAlt, faTimes, faChevronRight, faChevronLeft } from "@fortawesome/free-solid-svg-icons";
...
const ProjectModal = ({ project, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-center items-start md:items-center p-4 md:p-6 bg-black/60 backdrop-blur-sm transition-opacity overflow-y-auto" onClick={onClose}>
      <div 
      ...
      >
               ...
               {images.map((_, i) => (
                 <button 
                   ...
                   className={`w-2.5 h-2.5 rounded-full transition-all shadow-sm ${
                     i === currentImageIndex ? 'bg-indigo-500 scale-125' : 'bg-white/70 hover:bg-white'
                   }`}
                 />
               ))}
             </div>
        ...
        <div className="w-full h-auto p-6 md:p-8 lg:p-10 flex flex-col md:w-[55%] lg:w-[50%] md:overflow-y-auto custom-scrollbar">
          ...
          <div className="mb-10">
            <h4 className="text-sm font-bold text-gray-400 dark:text-gray-500 uppercase tracking-wider mb-4">Core Tech Stack</h4>
  ...
  const projectDetails = [
    {
      ...
      description: "OreChat is a real-time AI chatbot SaaS platform that allows businesses to deploy intelligent chat assistants on their websites. It combines automated AI responses directly powered by OpenAI & Google Gemini with seamless human-agent takeover.\n\nKey features include an intelligent knowledge-base query matcher, live chat widget injection via iframe, and a highly scalable Node/Socket.io backend crafted for high-performance production environments.",
      ...
      tech: [
        "React (Vite)", "Node.js", "Express.js", "Socket.io", "MongoDB", 
        "Redis", "BullMQ", "OpenAI API", "Google Gemini", "Cloudinary", 
        "Fuse.js", "JWT", "NGINX", "Tailwind CSS", "VPS Deployment"
      ],
    },
    {
      title: "Chat App",
      ...
      tech: ["React", "Node.js", "MongoDB", "Socket.io", "OTP Auth", "Tailwind CSS"],
    },
    {
      ...
      tech: ["React", "Node.js", "Express", "MongoDB", "JWT", "RichTextEditor", "Tailwind CSS", "SpeechSynthesis API"],
    },
    {
      title: "Note App",
      ...
      <div className="flex flex-wrap justify-center gap-8 px-4 md:px-8">
        {projectDetails.map((project, index) => (
          ...
          >
            ...
            <div>
              <div className="overflow-hidden rounded-[1.5rem] mb-6 relative shadow-md">
                 ...
                 <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500 rounded-[1.5rem] flex items-end p-5">
                    <span className="text-white font-bold tracking-widest text-xs uppercase transform translate-y-4 group-hover:translate-y-0 transition-transform duration-500">Explore Project</span>
                 </div>
              </div>
            ...
            >
              <span className="tracking-wide uppercase text-sm">View Details</span>
              <FontAwesomeIcon icon={faChevronRight} className="text-md transform group-hover:translate-x-2 transition-transform duration-300" />
            </button>
          </div>
        ))}
      </div>
